    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <div class="compact-table">
                ...
                <div class="container-input">
                    <div style="text-align: right"><b>Read</b></div>
                    <div><label><input type="checkbox" name="ur" value="1"<?php echo ($mode & 00400) ? ' checked' : '' ?>></label></div>
                    <div><label><input type="checkbox" name="gr" value="1"<?php echo ($mode & 00040) ? ' checked' : '' ?>></label></div>
                    <div><label><input type="checkbox" name="or" value="1"<?php echo ($mode & 00004) ? ' checked' : '' ?>></label></div>
                </div>
                <div class="container-input">
                    <div style="text-align: right"><b>Write</b></div>
                    <div><label><input type="checkbox" name="uw" value="1"<?php echo ($mode & 00200) ? ' checked' : '' ?>></label></div>
                    <div><label><input type="checkbox" name="gw" value="1"<?php echo ($mode & 00020) ? ' checked' : '' ?>></label></div>
                    <tdiv><label><input type="checkbox" name="ow" value="1"<?php echo ($mode & 00002) ? ' checked' : '' ?>></label></div>
                </div>
                <div class="container-input">
                    <div style="text-align: right"><b>Execute</b></div>
                    <div><label><input type="checkbox" name="ux" value="1"<?php echo ($mode & 00100) ? ' checked' : '' ?>></label></div>
                    <div><label><input type="checkbox" name="gx" value="1"<?php echo ($mode & 00010) ? ' checked' : '' ?>></label></div>
                    <div><label><input type="checkbox" name="ox" value="1"<?php echo ($mode & 00001) ? ' checked' : '' ?>></label></div>
                </div>
            </div>
...
<div class="block-file-info">
    <div class="block-file-info-wrapper">
        <div class="th checkbox-block"><label><input type="checkbox" title="Invert selection" onclick="checkbox_toggle()"></label></div>
        <div class="th name-file">Name</div>
        <div class="th info-size">Size</div>
        <div class="th change-file-time">Modified</div>
        <?php if (!FM_IS_WIN): ?>
        <div class="th">Perms</div>
        <div class="th">Owner</div><?php endif; ?>
        <div class="th block-action-file"></div>
    </div>
    <?php
    ...
    if ($parent !== false) {
        ...
        <div class="block-file-info-wrapper">
            <div class="td"></div>
            <div class="td"<?php echo !FM_IS_WIN ? '6' : '4' ?>"><a href="?p=<?php echo urlencode($parent) ?>"><i class="icon-arrow_up"></i> ..</a></div>
        </div>
        <?php
    }
    foreach ($folders as $f) {
        ...
        ?>
        <div class="block-file-info-wrapper">
            <div class="td "><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></div>
            <div class="td">
                <div class="filename">
                    <a href="?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">
                        <i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?>
                    </a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?>
                </div>
            </div>
            <div class="td">Folder</div>
            <div class="td"><?php echo $modif ?></div>
            <?php if (!FM_IS_WIN): ?>
            <div class="td"><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></div>
            <div class="td"><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></div>
            <?php endif; ?>
            <div class="td block-action-file">
    ...
    foreach ($files as $f) {
        ...
        ?>
        <div class="block-file-info-wrapper">
            <div class="td checkbox-block"><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></div>
            <div class="td name-file"><div class="filename"><a href="<?php echo fm_enc($filelink) ?>" title="File info"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div></div>
            <div class="td info-size"><span class="gray" title="<?php printf('%s bytes', $filesize_raw) ?>"><?php echo $filesize ?></span></div>
            <div class="td change-file-time"><?php echo $modif ?></div>
            <?php if (!FM_IS_WIN): ?>
            <div class="td"><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></div>
            <div class="td"><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></div>
            <?php endif; ?>
    ...
    if (empty($folders) && empty($files)) {
        ...
        <div class="block-file-info-wrapper">
            <div class="td"></div>
            <div class="td"><?php echo !FM_IS_WIN ? '6' : '4' ?>"><em>Folder is empty</em></div>
        </div>
        <?php
    } else {
        ?>
        <div class="block-file-info-wrapper">
            <div class="td gray checkbox-block"></div>
            <div class="td gray all-info-block" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
...
<style>
html,body,div,span,p,pre,a,code,em,img,small,strong,ol,ul,li,form,label,table,.block-file-info-wrapper,.th,.td {
    margin:0;
    padding:0;
...
html{overflow-y:scroll}body{padding:0;font:14px/16px Tahoma,Arial,sans-serif;color:#222;background: #fbfbfb;}
...
ul li{padding:3px 0}
.block-file-info{border-collapse:collapse;border-spacing:0;margin-bottom:10px;width:100%}
.th,.td{padding:7px 7px;text-align:left;vertical-align:top;white-space:nowrap}
.td.gray span{color:#222}
.block-file-info-wrapper:hover {
    background: #f9fafe;
    border: 1px solid #9bbcff;
}
.block-file-info-wrapper:last-child:hover {
    background: #fff;
    border: 1px solid #efefef;
}
/*.block-file-info-wrapper:hover .td.gray{background-color:#eee}*/
...
.icon-document{background-position:-16px 0}
.icon-folder{background-position:-32px 0}
.icon-folder_add{background-position:-48px 0}
.icon-upload{background-position:-64px 0}
.icon-arrow_up{background-position:-80px 0}
.icon-home{background-position:-96px 0}
.icon-separator{background-position:-112px 0}
.icon-cross{background-position:-128px 0}
.icon-copy{background-position:-144px 0}
.icon-apply{background-position:-160px 0}
.icon-cancel{background-position:-176px 0}
.icon-rename{background-position:-192px 0}
.icon-checkbox{background-position:-208px 0}
.icon-checkbox_invert{background-position:-224px 0}
.icon-checkbox_uncheck{background-position:-240px 0}
.icon-download{background-position:-256px 0}
.icon-goback{background-position:-272px 0}
.icon-folder_open{background-position:-288px 0}
.icon-file_application{background-position:0 -16px}
.icon-file_code{background-position:-16px -16px}
.icon-file_csv{background-position:-32px -16px}
.icon-file_excel{background-position:-48px -16px}
.icon-file_film{background-position:-64px -16px}
.icon-file_flash{background-position:-80px -16px}
.icon-file_font{background-position:-96px -16px}
.icon-file_html{background-position:-112px -16px}
.icon-file_illustrator{background-position:-128px -16px}
.icon-file_image{background-position:-144px -16px}
.icon-file_music{background-position:-160px -16px}
.icon-file_outlook{background-position:-176px -16px}
.icon-file_pdf{background-position:-192px -16px}
.icon-file_photoshop{background-position:-208px -16px}
.icon-file_php{background-position:-224px -16px}
.icon-file_playlist{background-position:-240px -16px}
.icon-file_powerpoint{background-position:-256px -16px}
.icon-file_swf{background-position:-272px -16px}
.icon-file_terminal{background-position:-288px -16px}
.icon-file_text{background-position:-304px -16px}
.icon-file_word{background-position:-320px -16px}
.icon-file_zip{background-position:-336px -16px}
.icon-logout{background-position:-304px 0}
.icon-chain{background-position:-320px 0}
.icon-link_folder{background-position:-352px -16px}
.icon-link_file{background-position:-368px -16px}
.compact-table{border:0;width:auto}
.compact-table .td,.compact-table .th{width:100px;border:0;text-align:center}
.compact-table .block-file-info-wrapper:hover {background-color:#fff}
